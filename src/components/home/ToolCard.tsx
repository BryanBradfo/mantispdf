...
export default function ToolCard({ title, description, href, icon, available }: ToolCardProps) {
  const card = (
    <div
      className={`h-full flex flex-col rounded-xl border p-6 transition ${
        available
          ? "border-gray-200 bg-white shadow-sm hover:border-mantis-300 hover:shadow-md"
          : "border-gray-100 bg-gray-50 opacity-60"
      }`}
    >
      <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-mantis-50 text-2xl">{icon}</div>
      <h3 className="text-lg font-semibold text-gray-900">{title}</h3>
      <p className="mt-1 flex-grow text-sm text-gray-500">{description}</p>
      {!available && (
